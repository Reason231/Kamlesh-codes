...
bannerRouter.route("/:id")
    ...
    // banner update
    .put(checkLogin,allowUser([UserTypes.ADMIN], setPath("/banner"), uploader.single("image"), bodyValidator(BannerCreateDto)), bannerController.update)  // here it requires the uplaoder cause we need to change the logo if somebody wants to update it

    // banner delete
    .delete(checkLogin, allowUser([UserTypes.ADMIN]), bannerController.delete)

bannerRouter.get('/:slug/detail' , bannerController.getBySlug)  // It is the different route which is required for slug
