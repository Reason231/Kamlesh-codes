...
const mystorage=multer.diskStorage({
    ...
    // second step that is naming file to a uniquenes
     filename: (req,file,callback)=>{                // file represnts the file that we have uploaded and filename is the name of file which we uplaoded   
        ...
        const ext=file.originalname.split(".").pop()       // ext represnts the extensionn which should be store in this variable because we want to filename unique but ext same
...
const uploader=multer({
    storage:mystorage,
    fileFilter:(req,file,callback)=>{             // third step to upload only image file
